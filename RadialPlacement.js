...
var firstElementSuffix = First_Element_TEdit.Text.split(/[A-Z]/);
var lastElementPrefix = Last_Element_TEdit.Text.split(/[0-9]/);
var lastElementSuffix =  Last_Element_TEdit.Text.split(/[A-Z]/);

var firstRoomNamePrefix = First_Room_Name_TEdit.Text.split(/[0-9]/);
var firstRoomSuffix = First_Room_Name_TEdit.Text.split(/[A-Z]/);
var lastRoomNamePrefix = Last_Room_Name_TEdit.Text.split(/[0-9]/);
var lastRoomSuffix =  Last_Room_Name_TEdit.Text.split(/[A-Z]/);
...
/*Automatic angle step calculation - 
  if(firstRoomNamePrefix != "")
    {
    angleStep = (endAngle - startAngle)/(((parseInt(lastRoomSuffix)+1)-parseInt(firstRoomSuffix))*((parseInt(lastElementSuffix)+1)-parseInt(firstElementSuffix)));
    }
    else
     {
     angleStep = (endAngle - startAngle)/((parseInt(lastElementSuffix)+1)-parseInt(firstElementSuffix));
     }
     ShowMessage(angleStep);
     */


for (j = firstRoomSuffix; j <= lastRoomSuffix; j++)
{
for(i = firstElementSuffix; i<=lastElementSuffix; i++)
    {
